                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('exams.update', $exam->id) }}">
                            ...
                            <div class="col-md-6">
                                <select class="form-control @error('student') is-invalid @enderror" name="student">
                                    ...
                                    @foreach ($students as $student)
                                        <option value="{{ $student->id }}" {{ $exam->student_id == $student->id ? 'selected' : ''}}>{{ $student->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            @error('student')
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-md-6">
                                <select class="form-control @error('term') is-invalid @enderror" name="term">
                                    ...
                                    <option value="One" {{ $exam->term == 'One' ? 'selected' : ''}}>One</option>
                                    <option value="Two" {{ $exam->term == 'Two' ? 'selected' : ''}}>Two</option>
                                </select>
                            </div>
                            @error('term')
                                <span class="invalid-feedback" role="alert">
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-md-6">
                                <input id="mark" type="text" class="form-control @error('mark.'.$subject->id) is-invalid @enderror" name = "mark[{{$subject->id}}]" value="{{ $mark[$key]['mark'] ?? old('mark.'.$subject->id) }}"  autocomplete="age">
